...
를 다운받으시거나 복사하셔서 `julia_example.py`로 생성해주고,<br>
- `@profile` 데코레이터를 통해 분석을 수행할 함수를 지정해 줍니다.
- 대부분 조건문과 반복문을 통한 연산입니다.
...
을 통해 속도를 드라마틱하게 향상시켰습니다.<br><br>

참고로, 주피터 노트북 환경에서는 셀 앞에다가 `%time`, `%timeit`을 추가하여 간단하게 측정할 수 있습니다.<br>
하지만 셀별로 측정하는거라 자세한 `profiling`을 하고 싶으면 `line_profiler`를 쓰는 것을 추천합니다!<br>
