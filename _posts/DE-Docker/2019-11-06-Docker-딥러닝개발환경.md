...
#### jupyter notebook 설치
<pre><code>docker run -it --rm -p 8888:8888 tensorflow/tensorflow:nightly-gpu-py3-jupyter
</code></pre>
...
<pre><code>docker run -it -p [host port]:[container port] -v ${HOME}/code:/tf [image-id]
</code></pre>
...
- 직접 설치
<pre><code>sudo add-apt-repository ppa:graphics-drivers/ppa # ppa를 추가
sudo apt update
...
<pre><code>docker run --runtime=nvidia --rm -ti nvidia/cuda nvidia-smi
</code></pre>
...
위에서 설치했으니 패스하면 된다!
<pre><code>docker pull tensorflow/tensorflow:nightly-gpu-py3-jupyter
</code></pre>

- 최종실행!
<pre><code>docker run --runtime=nvidia -it -p 2222:8888 tensorflow/tensorflow:nightly-gpu-py3-jupyter
</code></pre>

- volume 설정을 원하면 -v옵션을 추가하여 실행해주면 된다.
<pre><code>docker run --runtime=nvidia -it -p 2222:8888 -v /Users:/tf tensorflow/tensorflow:nightly-gpu-py3-jupyter
</code></pre>
