---
layout: page
title: About Me
permalink: /about/
feature-img: "assets/img/tree.jpg"
---

<style>
.text{text-indent: 50px;}
#jb-sidebar{width: 260px;padding: 20px;margin-bottom: 10px;float: left;border: 0px;text-align: right;}
#jb-content{width: 580px;padding: 20px;margin-bottom: 10px;float: right;border: 0px;}
@media ( min-width: 481px )
{
  #jb-sidebar { width: 260px; float: left; }
  #jb-content { width: 580px; float: right; }
}
</style>


<center>

<br>

<p style="text-align: left">제 블로그에 방문해 주셔서 감사합니다!</p><br>

<p style="text-align: left">안녕하세요, 저는 현재 국민대학교에서 공부하고 있는 최승호라고 합니다.</p><br>

<p style="text-align: left">이 블로그는 제가 공부하거나 몰랐던 과정을 게시하기 위해 개설하였습니다.</p><br>

<p style="text-align: left">최대한 알기 쉽게 작성하고 유용한 꿀팁들을 공유하는게 목표입니다.</p><br>

<p style="text-align: left">부족한 부분이 많겠지만 피드백 받아 점차 고쳐 나가겠습니다!</p><br>

<br>

</center>

---------------------------

> <h3 style="color: red">Skills</h3>

- Programming Language: `Python`, Java
- Python Library: Sckit learn, Keras, Pytorch, Pandas, Numpy
- Development Environment: Docker
- WEB Programming: Ruby on Rails, Django
- Certification: SQLD, 정보처리기사

---

> <h3 style="color: red">Experience</h3>

- `다이어`: ICT인턴십 - [2019.09 ~]
- `투빅스`: 빅데이터 분석 연합 동아리 - [2019. 07 ~ ]
- [`AI Innovation Square`](https://github.com/smothly/ai_innovation_NLP): 자연어 처리 심화 교육 - [2019. 05 ~ 2019. 06]()
- `이팝콘`: 학점연계인턴 [2018. 12 ~ 2019. 02]
- `멋쟁이 사자처럼`: 웹프로그래밍 연합 동이리 [2018. 04 ~ 2019 . 06]

---

> <h3 style="color: red">Project</h3>

- Web Project
  - [2018 국민대학교 축제페이지](https://github.com/smothly/KmuFestival)
  - [2018 멋쟁이사자처럼 해커톤](https://github.com/smothly/Hackaton_Platform_2018)

- Data Science Project
  - [욕설 탐지기](https://github.com/smothly/BadWordDetection)
  - [뉴스 분류기](https://github.com/smothly/ai_innovation_NLP/blob/master/5.28~29%20search%20engine.ipynb)

- Etc
  - [Hidato Puzzle](https://github.com/YoungsunCho/Algorithm_HidatoPuzzle)
  -
---

<br><br><br><br><br>
